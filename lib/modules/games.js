...
module.exports = {
	...
	init: function(cfg, cmd, data) {
		...
		.command("spa")
			.help("Spaghetti management")
			.sub("spawn")
					.help("Admin command to give player spaghetti.")
					.usage("<user> <amount>")
					.demand(["user", "text"])
					.permission(cmd.Permissions.General.ADMINISTRATOR)
					.on(function(msg, args) {
						var given = parseInt(args.get(1).o),
							points = data.guild(msg.guild).section("spaghetti").or(0).get(args.get(0).o.id);

						if (!given || given > 1000000) {
							msg.reply("Stahp. Too much spaghetti.");
							return;
						}

						if (given + points > 10000000) {
							msg.reply("You have wayyyyy too much spaghetti. Where are you carrying all of that?!?");
							return;
						}

						data.guild(msg.guild).section("spaghetti").write(args.get(0).o.id, points + given).commit();

						msg.reply("Gave " + args.get(0).o.mention + " " + given + " :spaghetti:.");
					}).bind()
				.sub("give")
					.help("Give another player some of your spaghetti.")
					.usage("<user> <amount>")
					.demand(["user", "text"])
					.on(function(msg, args) {
						var amount = parseInt(args.get(1).o),
							d = data.guild(msg.guild).section("spaghetti").or(0),
							p1 = d.get(msg.author.id),
							p2 = d.get(args.get(0).o.id);

						if (amount < 0) {
							msg.reply("You can't give someone negative spaghetti!");
							return;
						}

						d.write(msg.author.id, p1 - amount).write(args.get(0).o.id, p2 + amount).commit();

						msg.reply("Gave " + args.get(0).o.mention + " " + amount + " :spaghetti:");
					}).bind()
			.bind()
		.command("leaderboard")
		...
		.command("rps")
			...
			.sub("start")
				...
				.on(function(msg, args) {
					...
					if (args.get(0).o.id == cmd._.bot().User.id) {
						msg.reply("You can't challenge the bot to a game!");
						return;
					}
		...
		.command("trivia")
			...
			.sub("start")
				...
				.on(function(msg, args) {
					...
					if (rounds > 75) {
						rounds = 75;
						msg.reply("You can't have more than 75 rounds of trivia. Reduced to 75 rounds.");
					}
		...
		.command("slots")
			...
			.on(function(msg, args) {
				...
				if (amount < 0) {
					msg.reply("You can't bet with negative spaghetti!");
					return;
				}
